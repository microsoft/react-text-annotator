...
import { AnnotationData, ITokenStore, noopEmptyObject, noopUndefined, TokenEventListenersProp } from './types/labelerTypes';
...
export const Labeler = observer(
    React.forwardRef(<T extends ITokenStore>(props: LabelerProps<T>, labelerRef: React.MutableRefObject<LabelerRef>) => {
        ...
        React.useEffect(() => {
            const maxAnnotationIndex = annotations.reduce((mx, annotation) => Math.max(mx, annotation.endToken), 0);

            if (maxAnnotationIndex >= text.length) {
                annotationStore.setAnnotations([]);
                configStore.setAreAnnotationValid(false);
                console.error('Annotations end-token indices exceed the length of the given text');
            } else {
                configStore.setAreAnnotationValid(true);
            }
        }, [annotations, text]);
        ...
        useDeepCompareEffect(() => {
            if (!charToTokenMap?.size || !configStore.areAnnotationsValid) {
                return;
            }
